...
/*
	...
	Version LK1.20 (IC Plus) <Jesse Huang>
	- Support Prolific CPU
	
*/

#define DRV_NAME	"sundance"
#define DRV_VERSION	"1.30b"
#define DRV_RELDATE	"7-9-2007"
...
/* Set the copy breakpoint for the copy-only-tiny-frames scheme.
   ...
   need a copy-align. */
#define DEFAULT_COPYBREAK 4096
static int rx_copybreak=DEFAULT_COPYBREAK;//JESSE_FOR_PROLIFIC_ARM9
...
//Use memory mapping for default
...
struct netdev_private {
	...
	struct proc_dir_entry *ICPlus_NIC_dir, *IPF_EEPROM;
};
...
#ifdef NO_EEPROM
static u8 DefaultStationAddr[6]={0x00,0x05,0x00,0x19,0x00,0x33};
#endif

/*===========================================================================*/
#ifdef MP_EEPROM
#include "mp/mp_eeprom.c"
static int proc_read_IPFEEPROM(char *page, char **start,off_t off, int count,int *eof, void *data)
{
   ...
   for( i = 0; i < 3; i++) 
		mac_addr[i]=le16_to_cpu(eeprom_read(ioaddr, i + EEPROM_SA_OFFSET));
	for( i = 0; i < 6; i++) 
		len+=sprintf(page+len,"%2.2x",((unsigned char*)mac_addr)[i]);

	return len;
}
static int proc_write_IPFEEPROM(struct file *file,const char *buffer,unsigned long count,void *data)
{
   ...
   if(data == 0)return 0;
	...
	for( i = 0; i < 12; i++)
	{
		if ((mac_addr[i] >= 0x30) && (mac_addr[i] <= 0x39))/*0~9*/
		{
			mac_addr[i]=mac_addr[i]-'0';
		}else if ((mac_addr[i] >= 0x41) && (mac_addr[i] <= 0x46))/*A~F*/
		{
			mac_addr[i]=mac_addr[i]-'A'+10;
		}else if ((mac_addr[i] >= 0x61) && (mac_addr[i] <= 0x66))/*a~f*/
		{
			mac_addr[i]=mac_addr[i]-'a'+10;
		}else
		{
	...
	if(i != 12 )
	{
		printk("Invalid MAC address format\n");
		return len;
   }
	
	for( i = 0; i < 6; i++)
	   dev->dev_addr[i]=(mac_addr[i*2] << 4) + (mac_addr[i*2 + 1]); 
	mp_eeprom_updata(ioaddr,dev->dev_addr);
	__set_mac_addr(dev);

	return len;
}
static void ICP_CreateProc(struct net_device *dev)
{
	struct netdev_private *np = dev->priv;
	char dir_name[255]="ICPlus_";
	
	strcat(dir_name,dev->name);
	np->ICPlus_NIC_dir = proc_mkdir(dir_name, NULL);
	np->ICPlus_NIC_dir->owner = THIS_MODULE;

	np->IPF_EEPROM = create_proc_entry("IPF_EEPROM", 0777, np->ICPlus_NIC_dir);
	np->IPF_EEPROM->data=dev;
	np->IPF_EEPROM->read_proc = proc_read_IPFEEPROM;
	np->IPF_EEPROM->write_proc = proc_write_IPFEEPROM;
	np->IPF_EEPROM->owner= THIS_MODULE;
}
static void ICP_RemoveProc(struct net_device *dev)
{
	struct netdev_private *np = dev->priv;
	char dir_name[255]="ICPlus_";
	
	strcat(dir_name,dev->name);
	remove_proc_entry("IPF_EEPROM", np->ICPlus_NIC_dir);
...
/* when built into the kernel, we only print version if device is found */
	...
	dev->set_mac_address = sundance_set_mac_address;
	...
	for(i=0;i<6;i++)
	{
		dev->dev_addr[i]=DefaultStationAddr[i];
	}
	__set_mac_addr(dev);
	
	/* Enable TxLargeEnable and RxLargeEnable*/
	writew((readw(ioaddr+ASICCtrl)&0xffef)|0xc, ioaddr + ASICCtrl);
	
	/* Enable Auto-Cross-Over, Disable LDPS and LastGasp*/
	writeb((readb(ioaddr + 0x5f)&0xfd)|0x02, ioaddr + 0x5f);
	
	/* Wake On Lan Enable*/
	writeb(readb(ioaddr + 0x45)|0x80, ioaddr + 0x45);
	
	/* Write DSP parameter*/
	mdio_write(dev,np->phys[0],30,(mdio_read(dev,np->phys[0],30)&0xfbfe)|0x1000);
	mdio_write(dev,np->phys[0],29,(mdio_read(dev,np->phys[0],29)&0xf68e)|0x28e);
...
static int netdev_open(struct net_device *dev)
{
	...
	mdio_write (dev, np->phys[0], MII_BMCR, BMCR_PDOWN);
	mdelay(1);        
  
    writew(GlobalReset |FIFOReset, ioaddr +ASICCtrl + 2);
    for(i=2000;i >0;i--)
    {
  	 ...
  	 writew(GlobalReset | DMAReset | FIFOReset |NetworkReset|HostReset, ioaddr +ASICCtrl + 2);
     for(i=2000;i >0;i--)
     {
	 		  if((readw(ioaddr + ASICCtrl +2)&ResetBusy) == 0)
	   		break;
  ...
  init_ring(dev);   
	...
	mdio_write (dev, np->phys[0], MII_BMCR, BMCR_RESET|BMCR_FULLDPLX|BMCR_ANRESTART|BMCR_SPEED100);
        for(i=1000;i;i--)
	{
	   mdelay(1);
           if(mdio_read(dev,np->phys[0],MII_BMSR)&0x20)break;
...
reset_tx (struct net_device *dev)
{
	...
	writew (TxReset | DMAReset | FIFOReset | NetworkReset|HostReset,
			ioaddr + ASICCtrl + 2);
	for (i=50; i > 0; i--) {
		if ((readw(ioaddr + ASICCtrl + 2) & ResetBusy) == 0)
			break;
		...
		if (np->pci_rev_id >= 0x14) {
			...
			for (; np->cur_tx - np->dirty_tx > 0; np->dirty_tx++) {
				...
				if (sw_frame_id == hw_frame_id &&
					...
					& 0x00010000))
						break;
/*Jesse: this may have problem when hw quick than sw, and small tx-ring size, I am not sure.*/						
